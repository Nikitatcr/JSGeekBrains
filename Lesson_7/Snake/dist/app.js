class Board{
    ...
    /**
     * Метод отрисовывает доску с помощью добавления ячеек и столбцов
     */
    renderBoard() {
     ...
     * @returns {null|HTMLTableCellElement[]} возвращает набор тегов td если
     * были переданы координаты, иначе null
     */
...
class Food {
    ...
    /**
     * Метод устанавливает случайное положение еды на игровом поле.
     */
    setNewFood() {
    ...
    /**
     * Метод генерирует новый объект еды сослучайным положением на игровом поле
     * @returns {Food}
     */
...
class Game {
    ...
    /**
     * Этот метод запускается каждый интервал времени и осуществляет:
     ...
     * @return {boolean} если длина змейки достигла длины, нужной для выигрыши, тогда true, иначе false
     */
    isGameWon(){
    ...
    /**
     * В зависимости от нажатой кнопки (вверх, вниз, влево,вправо) вызывается соответствующий метод
...
class Menu {
     ...
     * событий клика на кнопки "Старт" и "Паузка"
...
class Settings {
     ...
     * @param {number} params.speed - скорость перемешения змейки
    ...
    init(params) {
        ...
        if(defaultParams.speed < 1 || defaultParams.speed > 10) {
            throw new Error('Неверные настройки, значение rowsCount должно быть в диапазоне [10,10]');
        }
        this.speed = defaultParams.speed;

        if(defaultParams.winLength < 5 || defaultParams.winLength > 50) {
            throw new Error('Неверные настройки, значение rowsCount должно быть в диапазоне [5,50]');
        }
