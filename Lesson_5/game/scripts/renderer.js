let renderer = {
    ...
    /**
     * Метод генерирует игровое поле на основении размеров в конфиге.
     * @return {string} сгенерированный html-код таблицы (игрового поля).
     */
    generateBoard() {
        let board = '';
    ...
    /**
     * Если ячейка с переданными координатами есть, то возвращается её объъект,
     * а иначе null
     * @param {{x:number, y:nmber}} position объект с координатами
     * @return {HTMLTableCellElement|null} объект ячейки, если есть, в противном случае null
     */
    getSquare(position) {
